...
const createBooks = async (req, res) => {
    try {
        ...
        if (!data.userId)
            return res
                .status(400)
                .json({ status: false, msg: "please provide userId" });
        ...
        if (userId != tokenId)
            return res
                .status(403)
                .send({ status: false, msg: "Not authorized" });
        ...
        if (!data.excerpt)
            return res
                .status(400)
                .json({ status: false, msg: "please provide excerpt" });
        ...
        if (data.excerpt === "")
            return res
                ...
                .send({
                    status: false,
                    msg: "please provide content in excerpt",
                });
        if (!data.userId)
            return res
                .status(400)
        ...
        if (!data.ISBN)
            return res
                .status(400)
                .json({ status: false, msg: "please provide ISBN" });
        ...
        if (!data.category)
            return res
                .status(400)
                .json({ status: false, msg: "please provide category" });
        ...
        if (!data.subcategory)
            return res
                .status(400)
                .json({ status: false, msg: "please provide subcatogory" });
        ...
        if (!data.releasedAt)
            return res
                ...
                .send({
                    status: false,
                    message: "releasedAt is a mendatory field",
                });
        ...
        if (!regexForIsbn.test(data.ISBN)) {
            return res
                ...
                .send({
                    status: false,
                    msg: `The ISBN ${data.ISBN} is Not valid.`,
                });
        }
        ...
        if (validator.isNumeric(data.title))
            return res
                ...
                .send({
                    status: false,
                    msg: "Book title cannot be numbers only",
                });
        object.title = data.title;
        if (validator.isNumeric(data.excerpt))
            return res
                .status(400)
                .send({
                    status: false,
                    msg: "Book excerpt cannot be numbers only",
                });
        object.excerpt = data.excerpt;
        if (!validator.isAlpha(data.subcategory, "en-US", { ignore: "-', " }))
            return res
                .status(400)
                .send({
                    status: false,
                    msg: "Book subcategory should be string only you can use (-,')",
                });
        object.subcategory = data.subcategory;

        //-----------------checking duplicay of title and ISBN----------------------------------------------------
        let checkDuplicate = await bookModel.find({
        ...
        if (checkDuplicate.length >= 1) {
            if (data.title.toLowerCase() == checkDuplicate[0].title) {
                return res
                    ...
                    .send({
                        status: false,
                        msg: "Book with this title already exist",
                    });
            } else {
                return res
                    .status(400)
...
const getBOOksBYQuery = async function (req, res) {
    try {
        ...
        if (Object.keys(data).length == 0) {
            let Allbooksdata = await bookModel
            ...
            if (book.length == 0)
                return res
                    .status(404)
                    .send({ status: false, msg: "no book found" });
    ...
    try {
        ...
        if (!book)
            return res
                ...
                .json({
                    status: false,
                    msg: "book does not exists with thid Id",
                });
...
const updateBook = async function (req, res) {
    try {
        ...
        if (!checkBook) {
            return res
                .status(404)
                .send({ msg: "book does not exists with thid Id" });
        }
        ...
        if (data.excerpt) {
            if (typeof data.excerpt != "string") {
                return res
                    .status(400)
                    .send({
                        msg: "please Enter data.excerpt in a string format",
                    });
            }
            data.excerpt = data.excerpt.trim();
        ...
        if (Object.keys(obj).length == 0)
            return res
                ...
                .send({
                    status: false,
                    msg: "please provide something to update",
                });
