...
let createUser = async (req, res) => {
    try {
        ...
        if (!data.email)
            return res
                .status(400)
                .send({ status: false, msg: "please provide email" }); //validate this
        ...
        if (data.password.length >= 8 && data.password.length <= 15) {
            ...
            return res.status(400).send({
                status: false,
                msg: "password must be of length 8-15",
            });
        }
        ...
        if (typeof data.address != "undefined") {
            if (typeof data.address != "object") {
                return res.status(400).send({
                    status: false,
                    message: "adress must be an object ",
                });
            }
        }
                ...
                if (data.address.pincode || data.address.pincode == "") {
                    ...
                    if (
                        ...
                        return res.status(400).send({
                            status: false,
                            message:
                                "make sure pincode should be numeric only and 6 digit number",
                        });
                }
            }
...
const loginuser = async function (req, res) {
    try {
        ...
        return res
            .status(200)
            .send({ status: true, msg: "Token is generated", data: token });
    } catch (error) {
